...
export const multipleSession = async (data) => {
  const conState = data.status;
  const { ipaddress } = data;
  const { username } = data;
  const { password } = data;
  const { monitorItems } = data;
  const changeValues = [];
  ...
  if (!conState) {
    ...
    //   {
    //     nodeId:
    //       'ns=4;s=|var|CODESYS Control for Raspberry Pi MC SL.Application.PLC_PRG.Y000',
    //     attributeId: AttributeIds.Value,
    //   },
    // ];
    ...
    // const monitoredItems = ClientMonitoredItem.create(
    //   subscription,
    //   itemsToMonitor,
    //   options
    //   // TimestampsToReturn.Both
    ...
    group.on('changed', (monitoredItem, dataValue, index) => {
      ...
      console.log('------------------------------------------');
      // mainWin().webContents.send('changeValue', dataValue);
      changeValues[index] = {
